...
Data = []
mode = types[0]  # Make sure 'types' is defined somewhere, e.g., ['l2']
...
for s in range(5):
    ...
    Result_norm = np.sum([np.linalg.norm(r - r / np.linalg.norm(r))**2 for r in Proj_X])
    All_result.append(Result_norm)
    print(f"Result norm: {Result_norm}, P_value:{P_values[s]}")
plt.figure()
