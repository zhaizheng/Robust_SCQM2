...
images_47 = images[mask][:50]   # 40 samples
labels_47 = labels[mask][:50]
...
# 3️⃣ LP_1 mode (p = 1)
# -------------------
Q_lin_lp1, Theta_lin_lp1, c_lin_lp1, taus_lin_lp1, _, _ = quadratic_manifold_factorization(
    X, d=d, s=s,
    eta_Q=1e-1, eta_Theta=1e-1, eta_c=1e-1, eta_tau=1e-1,
    T=1000, tol=1e-10, mode='lp_p', delta=0.1, p=1,
    set_Theta_zero=True
)
U_lin_lp1, V_lin_lp1 = Q_lin_lp1[:, :d], Q_lin_lp1[:, d:d+s]
F_lin_lp1, _, _, _ = forward_all(
    X, taus_lin_lp1, c_lin_lp1, U_lin_lp1, V_lin_lp1, Theta_lin_lp1
)

Q_quad_lp1, Theta_quad_lp1, c_quad_lp1, taus_quad_lp1, _, _ = quadratic_manifold_factorization(
    X, d=d, s=s,
    eta_Q=1e-1, eta_Theta=1e-1, eta_c=1e-1, eta_tau=1e-1,
    T=1000, tol=1e-10, mode='lp_p', delta=0.1, p=1,
    set_Theta_zero=False
)
U_quad_lp1, V_quad_lp1 = Q_quad_lp1[:, :d], Q_quad_lp1[:, d:d+s]
F_quad_lp1, _, _, _ = forward_all(
    X, taus_quad_lp1, c_quad_lp1, U_quad_lp1, V_quad_lp1, Theta_quad_lp1
    )
...
images_list = [
    ...
    F_lin_lp1.T.reshape(-1, 28, 28),
    F_quad_lp1.T.reshape(-1, 28, 28)
]
